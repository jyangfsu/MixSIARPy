"""Lake example with raw sources and a Secchi-depth continuous effect.
...
MIX_FILENAME = DATA_DIR / 'lake_consumer.csv'
SOURCE_FILENAME = DATA_DIR / 'lake_sources.csv'
DISCR_FILENAME = DATA_DIR / 'lake_discrimination.csv'

# Results are saved relative to the current Spyder/terminal working directory.
OUTPUT_DIR = Path("outputs/lake")
...
print(
    f"lake: model built ({mix['N']} mixtures, "
...
if sample:
    ...
    prediction = continuous_effect_prediction(
        fit, mix, source["source_names"]
    )
    prediction.to_netcdf(output_dir / "continuous_effect_predictions.nc")
    figure = plot_continuous_effect(
        prediction, output_dir / "continuous_effect.png"
    )
    plt.close(figure)
